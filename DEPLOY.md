...
2. **Решение 2:** Если проблема сохраняется, попробуйте использовать прямой вызов:
   ```
   npm install && node --loader tsx script/build.ts
   ```
   (требует Node.js 20.6+, Render использует Node.js 22.16.0)
3. **Решение 3:** Убедитесь, что все инструменты сборки (`tsx`, `esbuild`, `vite`) находятся в `devDependencies` (уже есть)
4. **Решение 4:** Если ничего не помогает, попробуйте альтернативный подход:
   ```
   npm install && npx --yes tsx script/build.ts
   ```
